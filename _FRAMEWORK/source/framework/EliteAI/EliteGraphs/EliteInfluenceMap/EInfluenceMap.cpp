...
void InfluenceMap::Update(float deltaTime)
{
	...
	for(int cellIndex{0}; cellIndex < (m_numColumns * m_numRows); ++ cellIndex)
	{
		...
		std::vector<int> indices = GetNeighboringIndices(cellIndex);
		...
		for (int i{0}; i< indices.size(); ++i)
		{
			...
			float sampledInflunce = m_InfluenceFrontBuffer[indices[i]];
			float cost = m_influenceCosts[i];

			float influence = sampledInflunce * expf(-cost * m_Decay);

			if (abs(influence) > abs(maxInfluence)) maxInfluence = influence;
		}

		float newInfluence = Lerp(maxInfluence, m_InfluenceBackBuffer[cellIndex], m_Momentum);
		m_InfluenceBackBuffer[cellIndex] = newInfluence;
